...
bool LookupTableFunction::readBinMappingFloat1(int rows, int cols, std::string mapping_file, cv::Mat& out_map)
{
	cv::Mat map(rows, cols, CV_64FC1, cv::Scalar(0));
	std::ifstream inFile(mapping_file, std::ios::in | std::ios::binary);
	if (!inFile) {
		std::cout << "error" << std::endl;
		out_map = map.clone();
...
bool LookupTableFunction::generate_pointcloud_depth(cv::Mat mask, cv::Mat unwrap, cv::Mat& depth, cv::Mat& point, cv::Mat xL_rotate_x1,cv::Mat xL_rotate_y1,cv::Mat rectify_R11,cv::Mat pattern_mapping1,float b) {


	cv::Mat R1 = rectify_R11.clone();
	...
	double R1_t8 = R1_t.at<double>(2, 1);
	double R1_t9 = R1_t.at<double>(2, 2);
	cv::Mat xL_rotate_y = xL_rotate_y1.clone();
	xL_rotate_y.convertTo(xL_rotate_y, CV_64F);
	cv::Mat xL_rotate_x = xL_rotate_x1.clone();
	xL_rotate_x.convertTo(xL_rotate_x, CV_64F);
	cv::Mat single_pattern_mapping = pattern_mapping1.clone();
	single_pattern_mapping.convertTo(single_pattern_mapping, CV_64FC1);


	int img_height = mask.rows;
	int img_width = mask.cols;

	cv::Mat disparity_map = cv::Mat(img_height, img_width, CV_64F, cv::Scalar(0));
	cv::Mat xyz_mat(img_height, img_width, CV_64FC3, cv::Scalar(NULL, NULL, NULL));
	...
	for (int yc = 0; yc < img_height; yc++) {
		...
		double* ptr_x = unwrap.ptr<double>(yc);
		double* ptr_Lx = xL_rotate_x.ptr<double>(yc);
		double* ptr_Ly = xL_rotate_y.ptr<double>(yc);

		double* ptr_depth = disparity_map.ptr<double>(yc);
		...
		for (int xc = 0; xc < img_width; xc++) {
			...
			float Xp = ptr_x[xc] * 1280 / (2 * CV_PI);
			float Xcr = ptr_Lx[xc];
			float Ycr = ptr_Ly[xc];
			double Xpr = Bilinear_interpolation(Xp, (Ycr + 1) * 2000, single_pattern_mapping);
			...
			float X_L = Z * Xcr * R1_t1 + Z * Ycr * R1_t2 + Z * R1_t3;
			float Y_L = Z * Xcr * R1_t4 + Z * Ycr * R1_t5 + Z * R1_t6;
			float Z_L = Z * Xcr * R1_t7 + Z * Ycr * R1_t8 + Z * R1_t9;


			if (ptr_m[xc] > 2 && Z_L > 10 && Z_L < 3000 && Xp > 0)
			{
				ptr_xyz[xc][0] = X_L;
				ptr_xyz[xc][1] = Y_L;
				ptr_xyz[xc][2] = Z_L;
	...
	point = xyz_mat.clone();
	depth = disparity_map.clone();

	return true;
}

bool LookupTableFunction::generate_pointcloud_depth_repetition(cv::Mat mask, cv::Mat unwrap, cv::Mat& depth, cv::Mat& point, cv::Mat xL_rotate_x1, cv::Mat xL_rotate_y1, cv::Mat rectify_R11, cv::Mat pattern_mapping1, float b) {
	...
	for (int yc = 0; yc < img_height; yc++) {
		...
		float* ptr_x = unwrap.ptr<float>(yc);
		double* ptr_Lx = xL_rotate_x.ptr<double>(yc);
		double* ptr_Ly = xL_rotate_y.ptr<double>(yc);

		double* ptr_depth = disparity_map.ptr<double>(yc);

		float* ptr_m = mask.ptr<float>(yc);
		cv::Vec3d* ptr_xyz = xyz_mat.ptr<cv::Vec3d>(yc);
