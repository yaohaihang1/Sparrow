...
public:
	...
	bool getStandardPlaneParam(float* ptr_point_cloud, uchar* bright, float* R, float* T) { return 0; };

	void setCalibrateParam(struct CameraCalibParam param){
		camera_calibration_param_ = param;
	};
	
	bool findCircleBoardFeature(cv::Mat img, std::vector<cv::Point2f>& points) { return 0; };
...
private:
	bool bilinearInterpolationFeaturePoints(std::vector<cv::Point2f> feature_points, std::vector<cv::Point3f>& point_3d, cv::Mat point_cloud) { return 0; };
 
	float Bilinear_interpolation(float x, float y, cv::Mat& mapping) { return 0; };
